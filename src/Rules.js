...
class FullHouse extends Rule {
  evalRoll = (dice) => {
    // check if there are 3,2 types of combinations in dice set
    return this.freq(dice).some((c) => c == 3) &&
      this.freq(dice).some((c) => c == 2)
      ? this.score
      : 0;
  };
}
...
class SmallStraight extends Rule {
  evalRoll = (dice) => {
    ...
    // small straight must be 4 different dice & must contain a 3 and a 4
    return d.size >= 4 &&
      d.has(3) &&
      d.has(4) &&
      ((d.has(1) && d.has(2)) ||
